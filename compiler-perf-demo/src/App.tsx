...
function CounterButton(props: { onClick: () => void }) {
  return (
    <button onClick={props.onClick} className="px-4 py-2 bg-neutral-700 text-white rounded hover:bg-neutral-600 border border-white/20">
      Increase count
    </button>
  );
}
...
function ColorPicker(props: { value: string; onChange: (value: string) => void }) {
  return <input type="color" value={props.value} onChange={(e) => props.onChange(e.target.value)} className="w-full h-12 cursor-pointer rounded border border-white/20 bg-neutral-700 p-1" />;
}
  ...
  return (
    <div className={`flex gap-8`}>
      ...
      <div className="flex flex-col p-4 border border-white h-64 w-96 gap-2">
        ...
        <span className="text-center text-neutral-200 font-light">(This component renders 10,000 boxes)</span>
...
function ParentComponent() {
  return (
    <div className="flex flex-col min-h-screen">
      <h1 className="text-2xl font-bold text-center py-8 absolute top-0 left-0 right-0">React Compiler Demo</h1>
